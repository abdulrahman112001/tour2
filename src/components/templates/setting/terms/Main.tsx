...
function Main() {
  const endpoint = `settings?collection=terms`;
  const { data, refetch, isSuccess, isFetching, isLoading } = useFetch<any>({
    endpoint: endpoint,
    queryKey: [endpoint],
  });

  const AllData = data?.data[2];
  ...
  const handelSubmit = (values: values_TP) => {
    const finalOut = {
      "value[terms]": values?.terms,
    };
    mutate({ ...finalOut, collection: "terms", _method: "PUT" });
  };
